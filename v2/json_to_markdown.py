...
def get_json_files(data_dir: str) -> list[str]:
    ...
    for root, _dirs, files in os.walk(data_dir):
        json_files.extend(
            str(Path(root) / file) for file in files if file.endswith(".json")
        )

    # Sort by page number for sequential processing
    json_files.sort(key=lambda x: extract_page_number(Path(x).name))

    return json_files


def process_json_file(file_path: str) -> str:  # noqa: C901
    ...
    try:
        ...
        if not isinstance(data, list):
            ...
            return f"<!-- Error: Invalid JSON structure in {fname} -->\n\n"
            ...
            if isinstance(item, dict):
                ...
                if text and text.strip():
                    ...
                    if category == "Title":
                        ...
                        # Clean up text and preserve line breaks,
                        ...
                        # Treat section-headers as level 3
                        # headings and strip any # patterns
                        cleaned_heading = (
                            text.strip().replace("\n", " ").replace("  ", " ")
                        )
                        # Remove any # patterns at the beginning
        ...
        return f"<!-- Error processing {Path(file_path).name}: {e!s} -->\n\n"
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python json_to_markdown.py"
            ' "A Brief History of Guernsey County, Ohio"'
        )
        return
    ...
    for i, json_file in enumerate(json_files, 1):
        print(f"Processing {i}/{len(json_files)}: {Path(json_file).name}")
