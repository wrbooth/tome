...
def test_outline_extraction(file_path):
    ...
    for heading in headings[:10]:  # Show first 10
        print(f"  Level {heading['level']}: {heading['title']} (p. {heading['page'] + 1})")
...
def test_typography_detection(file_path):
    ...
    for heading in headings[:10]:  # Show first 10
        method = heading.get('detection_method', 'typography')
...
def test_with_guernsey_pdf():
    ...
    outline_headings = test_outline_extraction(file_path)
    toc_headings = test_toc_parsing(file_path)
    typography_headings = test_typography_detection(file_path)
