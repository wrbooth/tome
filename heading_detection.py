...
def is_quality_heading(text: str, font_size: float, is_bold: bool) -> bool:
    ...
    noise_patterns = [
        r'^[^\w]*$',  # Only symbols
        r'^\d+$',     # Only numbers
        r'^[A-Z][A-Z\s]{0,7}$',  # Short ALL CAPS (8 chars or fewer, complements 8+ detection threshold)
...
def extract_headings_from_outline(file_path: str, doc: "fitz.Document | None" = None) -> List[Dict[str, Any]]:
    """
...
def extract_headings_from_toc_pages(file_path: str, doc: "fitz.Document | None" = None) -> List[Dict[str, Any]]:
    """
...
def extract_headings_by_typography(file_path: str, doc: "fitz.Document | None" = None) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        logger.warning("Could not extract headings by typography from %s: %s", file_path, e)
        return []
...
def merge_heading_detection_methods(file_path: str, doc: "fitz.Document | None" = None) -> List[Dict[str, Any]]:
    """
...
def detect_heading_patterns_from_file(file_path: str, doc: "fitz.Document | None" = None) -> List[Dict[str, Any]]:
    """
