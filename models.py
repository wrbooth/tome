...
class Chunk(BaseModel):
    ...
    headings_path: list[str] = Field(
        default_factory=list, description="Heading hierarchy for this chunk"
    )
...
class PassageDetail(BaseModel):
    ...
    headings_path: list[str] | None = Field(
        default=None, description="Heading hierarchy"
    )
...
class QueryAnalysis(BaseModel):
    ...
    query_type: str = Field(
        description=(
            "Classification: who, when, where, person, company, factoid, general"
        )
    )
    person: str | None = Field(
        default=None, description="Primary person referenced in query"
    )
    entities: QueryEntities = Field(default_factory=QueryEntities)
    expansions: list[str] = Field(
        default_factory=list, description="Search expansion terms"
    )
